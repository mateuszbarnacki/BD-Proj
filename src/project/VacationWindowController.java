...
public class VacationWindowController {
    ...
    public void addVacation(ActionEvent event) {
        ...
        if (result.isPresent() && (result.get() == ButtonType.OK)) {
            ...
            if (controller.processResult()) {
                AlertLoader alertLoader = new AlertLoader(Alert.AlertType.INFORMATION, "Zaktualizowano listę rekordów", "Dodanie urlopu");
                alertLoader.load();
                refreshListView();
            }
        }
    ...
    public void deleteVacation(ActionEvent event) {
        ...
        if (response.isPresent() && (response.get() == ButtonType.OK)) {
            Datasource.getInstance().deleteWorker(vacationsList.getSelectionModel().getSelectedItem().getId());
            refreshListView();
